import { r as reactExports } from './react-D-xSlSIM.js';
 ...
 */
const useGamepadNavigation = (isEnabled = true) => {
  const [focusedElement, setFocusedElement] = reactExports.useState(null);
  const [navigableElements, setNavigableElements] = reactExports.useState([]);
  const [isGamepadConnected, setIsGamepadConnected] = reactExports.useState(false);
  const [showFocusIndicator, setShowFocusIndicator] = reactExports.useState(false);
  const lastNavigationTime = reactExports.useRef(0);
  const buttonStates = reactExports.useRef({});
  
  const registerNavigableElements = reactExports.useCallback(() => {
    if (!isEnabled) return;
  ...
  const navigateDirection = reactExports.useCallback((direction) => {
    const now = Date.now();
  ...
  const activateElement = reactExports.useCallback(() => {
...
export { useGamepadNavigation as u };
//# sourceMappingURL=useGamepadNavigation-BcjfaxZ8.js.map
